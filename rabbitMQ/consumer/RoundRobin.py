...
SERVER_PORTS = [8181, 8282, 8383, 8484, 8585]
current_server_index = 0

def get_next_server_port():
    global current_server_index
    port = SERVER_PORTS[current_server_index]
    current_server_index = (current_server_index + 1) % len(SERVER_PORTS)
    return port

def callback(ch, method, properties, body):
    data = json.loads(body)
    server_port = get_next_server_port()
    url = f'http://13.209.214.53:{server_port}/submit'
    try:
